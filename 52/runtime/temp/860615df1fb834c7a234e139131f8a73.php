<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:58:"F:\wamp\www\tp\52/application/admin\view\editor\index.html";i:1470626475;}*/ ?>
<div class="l-segment l-border-0 example-layout">
    <div class="l-row-padding">
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
        <div class="l-col-2 m-input">
            <label for="">输入项</label>
            <input type="text">
        </div>
    </div>
    <div class="l-segment l-border-0">
        <script class="m-editor" type="text/plain" style="width:100%;height:200px;">sdsg</script>
    </div>
    <div class="l-row-padding">
        <div class="l-col-1 m-input l-ml-10">
            <input type="submit" value="提交" class="m-button m-submit">
        </div>
        <div class="l-col-1 m-input">
            <input type="reset" value="重置" class="m-button-default m-reset">
        </div>
    </div>
    <br>
    <div class="l-divider-100"></div>
    <br>


</div>